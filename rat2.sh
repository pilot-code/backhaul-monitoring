...
while true; do
  ...
  if [ -f /var/run/reboot-required ]; then
    if (( CURRENT_TIME - LAST_RESTART >= REBOOT_CHECK_INTERVAL )); then
      echo "$TIME_NOW 🔁 System requires reboot. Rebooting now..." >> "$LOG_FILE"
      reboot
    else
      echo "$TIME_NOW ⏳ Waiting before reboot..." >> "$LOG_FILE"
    fi
  fi
