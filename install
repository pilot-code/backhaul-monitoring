...
select mode in "iran (backhaul + monitoring)" "kharej (backhaul + monitoring)" "only monitoring"; do
    case $REPLY in
...
if [ "$SRV_TYPE" != "monitor" ]; then
    ...
    # دانلود از گیت‌هاب خودت، اگر نشد از سرور ایران
    if curl -L --fail -o "$FILE_NAME" "$MY_GITHUB_URL/$FILE_NAME"; then
        echo "Downloaded from your GitHub successfully."
    else
        echo "GitHub download failed. Trying from Iran mirror..."
        if curl -L --fail -o "$FILE_NAME" "$IRAN_URL/$FILE_NAME"; then
            echo "Downloaded from Iran server successfully."
        else
            echo "Error: Failed to download $FILE_NAME from both sources!"
            exit 1
        fi
    fi
    ...
    if tar -xzf "$FILE_NAME" -C /root/backhaul; then
        rm -f "$FILE_NAME"
        echo "Extraction successful, cleaned up file."
    else
    ...
    if [ "$SRV_TYPE" = "server" ]; then
        ...
        read -p "Enter Backhaul token: " BKTOKEN
        read -p "Enter ports for tunneling (comma separated, e.g. 8880,8080,2086,80): " PORTS_RAW
...
log_level = \"info\""
    ...
    echo "Config file created at /root/backhaul/config.toml"
...
echo "---------------------------------------------------"
read -p "How many minutes between each monitoring check? (default: 2) " MON_MIN
MON_MIN=${MON_MIN:-2}

# نصب مانیتورینگ (در همه حالت‌ها)
